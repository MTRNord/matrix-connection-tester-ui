...
:::warning
Once you set up well-known delegation and users start joining your server, changing it can be complex. Plan your delegation structure carefully before going into production.
:::

## How Well-Known Discovery Works
...
## Port 443 vs Port 8448

### Using Port 443 (Recommended)

Delegate to your Matrix server on port 443:
...
Delegate to port 8448 (Matrix's default federation port):
...
**Use this when:**
- You cannot use port 443 for Matrix
- Port 443 is already in use by another service
- You prefer to keep federation on a separate port

:::inset
**Best Practice:** Prefer port 443 with well-known delegation. This provides maximum compatibility and avoids firewall issues.
:::
...
**Problem:** Well-known file delegates back to itself or creates a loop.

**Solution:** Ensure your delegation target is different from your base domain, or points to an actual server endpoint.
...
If your Matrix server is on a subdomain, you still need well-known files on the root domain:

- `https://example.com/.well-known/matrix/server` (on root domain)
- Delegates to `matrix.example.com:443` (actual server location)

## Migration Scenarios

### Changing Server Location

If you need to move your Matrix server:

1. Set up new server at new location
2. Update well-known delegation to point to new location
3. Wait for DNS/cache propagation (24-48 hours)
4. Decommission old server

### Moving to Well-Known Delegation

If you're currently using port 8448 directly:

1. Set up reverse proxy on port 443
2. Add well-known delegation files
3. Test thoroughly before removing port 8448
4. Monitor for a week before closing port 8448
